...
def main():
    ...
    for num, sample_name in enumerate(sample_list):
        ...
        scan_dataset = NpyDataset(test_sample_dir, num_classes=4)
        ...
        scan_per_class_dice_Score = np.mean(list_perclass_dice_scores_batch, axis=0)
        ...
        if args.use_wandb:
            ...
            wandb.log({"seg loss per scan": scan_seg_loss})
            wandb.log({"cross entropy loss per scan": scan_ce_loss})
            wandb.log({"mean_dice_scores per scan": np.mean(mean_dice_scores)})  # Log the latest Dice score
            wandb.log({"ALL_dice_scores per scan": scan_per_class_dice_Score})  # Log the latest Dice score for each class

        print(
            f'Time: {datetime.now().strftime("%Y%m%d-%H%M")}, Seg Loss of scan: {scan_seg_loss}, CE_Loss of scan: {scan_ce_loss}, Loss of scan: {loss}, 'f'mean_dice_scores of scan: {np.mean(mean_dice_scores)}, 'f'Dice Score each class of scan: {scan_per_class_dice_Score}'
        )
    ...
    threshold =0.01
    mean_PER_CLASS_SEG_LOSS = mean_excluding_below_threshold_axis(PER_CLASS_SEG_LOSS, threshold, axis=0)
    mean_PER_CLASS_CE_LOSS = mean_excluding_below_threshold_axis(PER_CLASS_SEG_LOSS, threshold, axis=0)
    mean_PER_CLASS_LOSS = mean_excluding_below_threshold_axis(PER_CLASS_LOSS, threshold, axis=0)
    mean_PER_CLASS_DICE_SCORE = mean_excluding_below_threshold_axis(PER_CLASS_DICE_SCORE, threshold, axis=1)
    print(
        f'Time: {datetime.now().strftime("%Y%m%d-%H%M")}, Avg Seg Loss: {mean_PER_CLASS_SEG_LOSS}, CE_Loss: {mean_PER_CLASS_CE_LOSS}, Avg Loss: {mean_PER_CLASS_LOSS}, 'f'Avg Dice Score: {mean_PER_CLASS_DICE_SCORE}'
    )    

def mean_excluding_below_threshold_axis(array, threshold, axis):
    """
    Compute the mean of the array along a specified axis, excluding values below a certain threshold.

    Parameters:
    - array (np.ndarray): The input array.
    - threshold (float): The threshold value below which numbers are excluded.
    - axis (int): The axis along which to compute the mean.

    Returns:
    - np.ndarray: The mean of the array excluding values below the threshold along the specified axis.
    """
    # Create a boolean mask where values are greater than or equal to the threshold
    array = np.array(array)  # Convert list to numpy array if it's not already
    mask = array >= threshold
    # Apply the mask to filter out values below the threshold
    filtered_array = np.where(mask, array, np.nan)  # Replace values below threshold with NaN
    # Compute the mean along the specified axis, ignoring NaNs
    mean_value = np.nanmean(filtered_array, axis=axis)
