...
def main():
    ...
    for num, sample_name in enumerate(sample_list):
        ...
        if args.use_wandb:
            ...
            wandb.log({"ALL_dice_scores per scan": np.mean(dice_scores, axis=0)})  # Log the latest Dice score for each class

        print(
            f'Time: {datetime.now().strftime("%Y%m%d-%H%M")}, Seg Loss: {scan_seg_loss}, CE_Loss: {scan_ce_loss}, Loss: {loss}, 'f'Dice Score: {np.mean(mean_dice_scores)}'
        )
