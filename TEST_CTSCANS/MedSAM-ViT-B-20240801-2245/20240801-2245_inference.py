...
def main():
    ...
    print(
            f'Time: {datetime.now().strftime("%Y%m%d-%H%M")}, Avg Seg Loss: {np.mean(PER_CLASS_SEG_LOSS)}, CE_Loss: {np.mean(scan_ce_loss)}, Avg Loss: {np.mean(PER_CLASS_CE_LOSS)}, 'f'Avg Dice Score: {np.mean(PER_CLASS_DICE_SCORE)}'
        )    
