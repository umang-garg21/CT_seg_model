...
def main():
    ...
    for num, sample_name in enumerate(sample_list):
        ...
        scan_per_class_dice_Score = np.mean(dice_scores, axis=0)
        ...
        print(
            f'Time: {datetime.now().strftime("%Y%m%d-%H%M")}, Seg Loss: {scan_seg_loss}, CE_Loss: {scan_ce_loss}, Loss: {loss}, 'f'Dice Score: {np.mean(mean_dice_scores)}, 'f'Dice Score each class: {scan_per_class_dice_Score}'
        )
