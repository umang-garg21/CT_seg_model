...
def main():
    ...
    csv_file_path = args.scan_test_save_path+'log_file.csv'

    # Open the CSV file in write mode and write the header
    # This should be done only once, so it's outside the loop
    write_header = not os.path.exists(csv_file_path)
    with open(csv_file_path, 'a', newline='') as csvfile:
        fieldnames = [
            'Image Name', 'Seg Loss of Scan', 'CE Loss of Scan', 'Loss of Scan',
            'Mean Dice Scores of Scan', 'Dice Score Each Class of Scan'
        ]
    ...
    for num, sample_name in enumerate(sample_list):
        ...
        scan_per_class_dice_Score = mean_excluding_below_threshold_axis(dice_scores_batch, 0.01, axis=0)
        
        PER_CLASS_SEG_LOSS.append(scan_seg_loss)
        PER_CLASS_CE_LOSS.append(scan_ce_loss)
        PER_CLASS_LOSS.append(net_loss)
        PER_CLASS_DICE_SCORE.append(dice_scores_batch)
    ...
    mean_PER_CLASS_CE_LOSS = mean_excluding_below_threshold_axis(PER_CLASS_CE_LOSS, threshold, axis=0)
    mean_PER_CLASS_LOSS = mean_excluding_below_threshold_axis(PER_CLASS_LOSS, threshold, axis=0)
    Avg_PER_CLASS_DICE_SCORE = mean_excluding_below_threshold_axis(PER_CLASS_DICE_SCORE, threshold, axis=0)
    
    print(
        f'Time: {datetime.now().strftime("%Y%m%d-%H%M")}, Avg Seg Loss: {mean_PER_CLASS_SEG_LOSS}, CE_Loss: {mean_PER_CLASS_CE_LOSS}, Avg Loss: {mean_PER_CLASS_LOSS}, 'f'Avg Dice Score: {Avg_PER_CLASS_DICE_SCORE}'
    )    
