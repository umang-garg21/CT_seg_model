...
class NpyDataset(Dataset):
    def __init__(self, data_root, bbox_shift=20, num_classes =4):
        ...
        img_files = self.sort_files_numerically(img_files)
        self.img_path_files = [os.path.join(self.img_path, file) for file in img_files]

        # List and sort ground truth files numerically
        gt_files = [f for f in os.listdir(self.gt_path) if f.endswith('.npy')]
        gt_files = self.sort_files_numerically(gt_files)
        self.gt_path_files = [os.path.join(self.gt_path, file) for file in gt_files]

        print("IMG PATH FILES SORTED", img_files)
        print("GT PATH FILES", gt_files)
...
def main():
    ...
    print(
            f'Time: {datetime.now().strftime("%Y%m%d-%H%M")}, Avg Seg Loss: {np.mean(PER_CLASS_SEG_LOSS)}, CE_Loss: {np.mean(PER_CLASS_CE_LOSS)}, Avg Loss: {np.mean(PER_CLASS_CE_LOSS)}, 'f'Avg Dice Score: {np.mean(PER_CLASS_DICE_SCORE)}'
        )    
